...
function createVehicleCards(vehicles) {
  ...
  vehicles.forEach(vehicle => {
    const card = `
      <div class="group relative" data-hash="${vehicle.hash}" data-price="${vehicle.price}">
        ...
        <img id="vehicleImage" src="img/${vehicle.image}" alt="${vehicle.name}" class="h-full w-full object-cover object-center">
        </div>
        <h3 class="mt-4 text-sm text-gray-700">${vehicle.name}</h3>
        <p class="mt-1">$ ${vehicle.price.toLocaleString()},-</p>
  ...
  actualRentButtons.forEach(button => {
    button.addEventListener('click', (event) => {
      ...
      $.post('https://qb-rental/rent', JSON.stringify({ vehicleHash, vehiclePrice }));
    });
  });
}
